...
class SkipGramNeg(nn.Module):
    ...
    def forward_noise(self, batch_size: int, n_samples: int) -> torch.Tensor:
        ...
        # Move noise words tensor to the correct device
...
class NegativeSamplingLoss(nn.Module):
    ...
    def forward(
        ...
        # TODO
        out_loss: torch.Tensor = torch.log(
            torch.sigmoid(torch.sum(output_vectors * input_vectors, dim=1))
        )

        # Compute log-sigmoid loss for incorrect classifications
        # TODO
        noise_loss: torch.Tensor = torch.log(
            torch.sigmoid(
                -torch.bmm(noise_vectors, input_vectors.unsqueeze(2)).squeeze(2)
            )
        )

        # Compute final loss: Negative sum of correct and noisy log-sigmoid losses, averaged over the batch
        total_loss: torch.Tensor = -(out_loss + noise_loss.sum(dim=1)).mean()
