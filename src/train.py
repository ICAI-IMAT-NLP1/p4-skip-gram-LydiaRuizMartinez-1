...
def train_skipgram(
    ...
    for epoch in range(epochs):
        for input_words, target_words in get_batches(words, batch_size, window_size):
            ...
            noise_vectors: torch.Tensor = model.forward_noise(
                batch_size=len(inputs), n_samples=5
            )

            # negative sampling loss
            # TODO
            loss: torch.Tensor = criterion(input_vectors, output_vectors, noise_vectors)
